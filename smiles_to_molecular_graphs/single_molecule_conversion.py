...
def process(smiles, mol_dict = False):
    if rdkit is None:
    ...
    if simple == False:
        ...
        x = torch.cat( [x1.to(torch.float), x3.to(torch.float),x4.to(torch.float)], dim=-1)
    ...
    if simple == False:
        ...
        mol_dict = [*atom_dict, *neighbours_dict, *hydrogen_dict]
